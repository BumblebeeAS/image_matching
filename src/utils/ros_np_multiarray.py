# Adapted from https://github.com/neka-nat/ros_np_multiarray for ROS 2 Compatibility
# See https://github.com/neka-nat/ros_np_multiarray/issues/4
...
def _numpy_to_multiarray(multiarray_type, np_array):
    ...
    for i, (name, size, stride) in enumerate(
        (("dim%d" % i, shape[i], shape[i] * itemsize) for i in range(np_array.ndim))
    ):
