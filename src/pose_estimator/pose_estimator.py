...
import logging
import cv2
from feature_matcher.keypoints_match_producer import KeypointsMatchProducer, get_keypoints_match_producer
from feature_matcher.tools import create_save_image, create_show_image, plot_matches, time_func
...
class PoseEstimator:
    ...
    def register_template(self, name, world_dimensions, template_img):
        ...
        self.templates[name] = (np.array(world_dimensions), np.array(template_img.shape[1::-1])) # width, height

    @staticmethod
    def draw_object_points(img, object_points, R, t, camera):
        imgpts, _ = cv2.projectPoints(object_points, R, t, camera.camera_matrix(), camera.dist_coeffs())
    ...
    def compute_pose(self, img, template, camera_frame, *, lxtyrxby=None, debug=False, logger=None):
        if template is None:
        ...
        keypoints1, keypoints2 = self.keypoints_match_producer.process_image(img, template, debug, num_keypoints=20, lxtyrxby=lxtyrxby, logger=logger)
        ...
        object_coord = (keypoints1.keypoints - source_image_size / 2) * source_dimensions / source_image_size
        ...
        if debug:
            ...
            object_rect = np.array([[-_x, -_y, 0], [_x, -_y, 0], [_x, _y, 0], [-_x, _y, 0]])
            # object_rect = np.array([[0, 0, 0], [_x, 0, 0], [_x, _y, 0], [0, _y, 0]])
            img = self.draw_object_points(img, object_rect, R, t, camera)
            img = plot_matches(self.keypoints_match_producer.get_template(template).img, img,
                    keypoints1.keypoints, kp2, keypoints1.scores)
            self.visualize(img)
...
if __name__ == "__main__":
    ...
    camera = PinholeCamera("auv4/front_cam", 768, 492, 407.0646129842357, 407.0646129842357, 384.5, 246.5, 0, 0, 0, 0, 0)
    pose_estimator_1.register_camera(camera)

    
    pose_estimator_1.visualize_callbacks.append(create_show_image(pose_estimator_1.__class__.__name__))
    ...
    for i, file in enumerate(os.listdir(folder_path)):
        try:
            ...
            #0.48s
            rot, trans = pose_estimator_1.compute_pose(img, "Bootlegger", "auv4/front_cam", lxtyrxby=(l, t, r, b) if CROP_IMAGES else None, debug=True)
