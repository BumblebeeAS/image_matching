...
class BasicPoseEstimator:
    def __init__(self, input_topic, image_match_producer, visualization_topic, camera_info, template, template_path, template_dimensions, detected_objects_topic=None):
        ...
        self.pose_estimator.visualize_callbacks.append(lambda img: self.visualization_pub.publish(
            self.bridge.cv2_to_compressed_imgmsg(img, "jpeg")))
        self.pose_estimator.register_template(template, template_dimensions, cv2.imread(template_path))
    ...
    def cropped_image_callback(self, img_msg, detected_objects=None, debug=False):
        ...
        if self.CROP_IMAGES and detected_objects is not None:
            if any([x.name == self.template for x in detected_objects.detected]):
                detected_object = sorted(
                    detected_objects.detected, key=lambda x: x.extra[0], reverse=True)[0]
        ...
        if rot is not None and trans is not None and trans[2] > 0:
            self.publish_tf(rot, trans, img_msg.header.frame_id, "Bootlegger", img_msg.header.stamp)
...
if __name__ == "__main__":
    ...
    camera_info_topic = rospy.get_param("~camera_info_topic", "/auv4/front_cam/camera_info")
    ...
    if matcher == "coarse_loftr":
        image_match_producer = get_keypoints_match_producer(None, "coarse_loftr", {"debug": True}, {"debug": True})
    elif matcher == "sift_flann":
        image_match_producer = get_keypoints_match_producer("sift", "flann", {"debug": True}, {"debug": True})
    elif matcher == "sift_bf":
        image_match_producer = get_keypoints_match_producer("sift", "bf", {"debug": True}, {"debug": True})
    elif matcher == "superpoint_bf":
        image_match_producer = get_keypoints_match_producer("superpoint", "bf", {"debug": True}, {"debug": True})
    elif matcher == "superpoint_superglue":
        image_match_producer = get_keypoints_match_producer("superpoint", "superglue", {"debug": True}, {"debug": True})
    elif matcher == "fast_bf":
        image_match_producer = get_keypoints_match_producer("fast", "bf", {"debug": True}, {"debug": True})

    pose_estimator = BasicPoseEstimator(camera_topic,
                        image_match_producer,
                        visualization_topic,
                        camera_info,
                        template,
                        template_path,
                        (0.6096, 1.2192), # TODO: get the template dimensions from launch file or service or json file
